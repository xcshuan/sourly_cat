...
fn test_nft_transfer() {
    ...
    let lock_script_dep = CellDep::new_builder()
        ...
        .build();
    // prepare scripts
    let type_script = context.build_script(&out_point, lock_args).expect("script");
    let type_script_dep = CellDep::new_builder().out_point(out_point).build();
...
fn test_nft_fighting() {
    ...
    let owner_lock_args = random_20bytes();
    // prepare scripts
    ...
    let type_script = context
        .build_script(&out_point, owner_lock_args)
        .expect("script");
    let type_script_dep = CellDep::new_builder().out_point(out_point).build();
    ...
    let lock_args_1 = Vec::from(lock_script_1.args().as_slice());
    let lock_args_2 = Vec::from(lock_script_2.args().as_slice());
    //第一个NFT
    let mut conc: Vec<u8> = Vec::with_capacity(23);
    //名字125
    input_nft[0].name[0..4].copy_from_slice(b"125$");
    conc.extend(b"125".iter());
    conc.extend(lock_args_1.iter());
