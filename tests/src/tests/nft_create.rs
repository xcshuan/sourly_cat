...
fn gen_tx_for_create_nft(context: &mut Context, lock_args: Bytes) -> TransactionView {
    //load_script_bin
    ...
    let owner_lock_script = context
        ...
        .expect("lock script");
    let owner_lock_hash = owner_lock_script.calc_script_hash().as_bytes();
    ...
    let type_script = context
        .build_script(&sourly_cat_out_point, owner_lock_hash)
        .expect("script");
...
fn test_create_nft() {
    ...
    let tx = gen_tx_for_create_nft(&mut context, pubkey_hash);
